<?php
/*
 * @version $Id: mailcollector.class.php 12213 2010-08-30 13:34:44Z yllen $
 -------------------------------------------------------------------------
...
class MailCollector  extends CommonDBTM {
   ...
   **/
   ...
   function buildTicket($i,$options=array()) {
      ...
      $glpi_message_match = "/ITvision-([0-9]+)\.[0-9]+\.[0-9]+@\w*/";
      ...
      // See in title
      if (!isset($tkt['tickets_id']) && preg_match('/\[ITvision #(\d+)\]/', $head['subject'], $match)) {
         $tkt['tickets_id'] = (int)$match[1];
      }
      // Found ticket link
      if (isset($tkt['tickets_id'])) {
