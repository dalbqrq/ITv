...
sed -i '/^#command/Id' /etc/nagios/nrpe.cfg
PROC=(`ps aux |wc -l`)
WARPROC=(`expr $PROC + 100`)
CRITPROC=(`expr $PROC + 200`)
...
command[check_total_procs]=/usr/local/nagios/libexec/check_procs -w $WARPROC -c $CRITPROC
EOF
...
#grep ^command /etc/nagios/nrpe.cfg |awk -F "check" '{print $3}' |sed -e 's/_//g' >> /tmp/message.txt
