...
function location_tree:select_location_tree(id)
   local clause = ""
   if id then
      clause = "location_tree_id = "..id
   end
...
function list(web)
   local A = location_tree:select_location_tree()
...
function show(web, id)
   local A = location_tree:select_location_tree(id)
...
function update(web, id)
   local A = {}
   if id then
      local tables = "itvision_location_tree"
      local clause = "location_tree_id = "..id
      --A:new()
...
function render_show(web, A)
   ...
   res[#res + 1] = p{ button_link(strings.add, web:link("/add")) }
   res[#res + 1] = p{ button_link(strings.remove, web:link("/remove/"..A.location_tree_id)) }
   res[#res + 1] = p{ button_link(strings.edit, web:link("/edit/"..A.location_tree_id)) }
...
function render_remove(web, A)
   ...
   if A then
      A = A[1]
      url_ok = web:link("/delete/"..A.location_tree_id)
