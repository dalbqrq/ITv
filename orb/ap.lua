...
ap:dispatch_post(update, "/update/(%d+)")


function add(web)
   return render_add(web)
end
ap:dispatch_get(add, "/add")
...
ap:dispatch_post(insert, "/insert")
...
ap:dispatch_get(remove, "/remove/(%d+)")
...
ap:dispatch_get(delete, "/delete/(%d+)")


ap:dispatch_static("/css/%.css", "/script/%.js")
