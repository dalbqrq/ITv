...
--         (select object_id from nagios_objects o where a.id = o.name2 and name1 = ']]..config.monitor.check_app..[[') 
--         where a.service_object_id is null ]]
   ...
   for i,v in ipairs(app) do
      ...
      local ao = { app_id = root[1].app_id,  instance_id = config.database.instance_id, service_object_id = v.object_id, type = 'app' }
